...
const COLORS = {
  pink: "#d4537e", pinkLight: "#fbeaf0", pinkMid: "#ed93b1", pinkDark: "#72243e",
...
const INITIAL_DATA = {
  users: [
    { id: 1, nombre: "Admin", usuario: "admin", password: "admin123", email: "admin@nikibeauty.com", rol: "admin", localId: null, activo: true },
    { id: 2, nombre: "Laura Gómez", usuario: "laura", password: "laura123", email: "laura@nikibeauty.com", rol: "manicura", localId: 1, activo: true },
    { id: 3, nombre: "Sofía Torres", usuario: "sofia", password: "sofia123", email: "sofia@nikibeauty.com", rol: "manicura", localId: 1, activo: true },
    { id: 4, nombre: "Valentina Ruiz", usuario: "vale", password: "vale123", email: "vale@nikibeauty.com", rol: "manicura", localId: 2, activo: true },
  ],
  locales: [
    { id: 1, nombre: "Niki - Palermo", direccion: "Av. Santa Fe 3500" },
    { id: 2, nombre: "Niki - Belgrano", direccion: "Av. Cabildo 1200" },
  ],
  horarios: [], asistencias: [], periodosBloqueados: [],
  resetTokens: [], // { token, userId, expiry }
};

const STORAGE_KEY = "niki_app_data";
async function loadFromStorage() {
  try { const r = await window.storage.get(STORAGE_KEY); return r ? JSON.parse(r.value) : INITIAL_DATA; }
...
function getDiasDelMes(year, month) {
  const dias = []; const d = new Date(year, month, 1);
  while (d.getMonth()===month) { if (d.getDay()!==0) dias.push(new Date(d)); d.setDate(d.getDate()+1); }
  return dias;
}
function getSemanas(dias) {
  const semanas=[]; let sem=[];
  dias.forEach((d,i)=>{ sem.push(d); if(d.getDay()===6||i===dias.length-1){semanas.push([...sem]);sem=[];} });
...
function Btn({children,onClick,variant="primary",size="md",disabled,style}){
  const base={border:"none",borderRadius:8,cursor:disabled?"not-allowed":"pointer",fontWeight:500,display:"inline-flex",alignItems:"center",gap:6,opacity:disabled?0.5:1,transition:"all 0.15s",...style};
  const v={
...
function Login({onLogin,data,setData}){
  ...
  const [vista,setVista]=useState("login"); // login | recuperar | token | nueva
  const [email,setEmail]=useState(""); const [token,setToken]=useState("");
  const [nueva,setNueva]=useState(""); const [nueva2,setNueva2]=useState("");
  const [msg,setMsg]=useState(""); const [tokenValido,setTokenValido]=useState(null);

  const handleLogin=()=>{
    const found=data.users.find(x=>x.usuario===u.trim()&&x.password===p&&x.activo);
    if(found) onLogin(found); else setErr("Usuario o contraseña incorrectos.");
  };

  const handleRecuperar=()=>{
    const user=data.users.find(x=>x.email&&x.email.toLowerCase()===email.trim().toLowerCase()&&x.activo);
    if(!user){setMsg("Si el mail existe, vas a recibir las instrucciones.");return;}
    const tk=genToken();
    const expiry=Date.now()+30*60*1000; // 30 min
    setData(d=>({...d,resetTokens:[...(d.resetTokens||[]).filter(t=>t.userId!==user.id),{token:tk,userId:user.id,expiry}]}));
    // En producción esto enviaría un mail. Acá mostramos el token para demo.
    setMsg(`Demo: tu token es → ${tk}`);
  };

  const handleVerificarToken=()=>{
    const now=Date.now();
    const tk=data.resetTokens?.find(t=>t.token===token.trim()&&t.expiry>now);
    if(tk){setTokenValido(tk);setVista("nueva");setMsg("");}
    else setMsg("Token inválido o vencido.");
  };

  const handleNuevaPassword=()=>{
    if(!nueva||nueva!==nueva2){setMsg("Las contraseñas no coinciden.");return;}
    setData(d=>({
      ...d,
      users:d.users.map(u=>u.id===tokenValido.userId?{...u,password:nueva}:u),
      resetTokens:(d.resetTokens||[]).filter(t=>t.token!==tokenValido.token)
    }));
      ...
      <Card style={{width:"100%",maxWidth:360}}>
        ...
        {vista==="login"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <Btn onClick={handleLogin} style={{width:"100%",justifyContent:"center"}}>Ingresar</Btn>
        ...
        {vista==="recuperar"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <Btn onClick={handleRecuperar} style={{width:"100%",justifyContent:"center"}}>Enviar token</Btn>
        ...
        {vista==="token"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <Btn onClick={handleVerificarToken} style={{width:"100%",justifyContent:"center"}}>Verificar</Btn>
        ...
        {vista==="nueva"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {msg&&<p style={{margin:0,fontSize:13,color:COLORS.danger}}>{msg}</p>}
            <Btn onClick={handleNuevaPassword} style={{width:"100%",justifyContent:"center"}}>Guardar contraseña</Btn>
          </div>
        )}

        <p style={{fontSize:12,color:"var(--color-text-secondary)",marginTop:16,textAlign:"center"}}>Demo: admin/admin123 · laura/laura123</p>
      </Card>
    </div>
  );
}
...
// ── ABM MANICURAS ──────────────────────────────────────────────────
function ABMManicuras({data,setData}){
  const [modal,setModal]=useState(null); const [form,setForm]=useState({}); const [formErr,setFormErr]=useState("");
  ...
  const save=()=>{
    ...
    if(modal==="new"){
      ...
      const nu={nombre:form.nombre.trim(),usuario:form.usuario.trim(),email:form.email.trim(),password:form.password,rol:"manicura",localId:parseInt(form.localId)||null,activo:true,id:Date.now()};
      setData(d=>({...d,users:[...d.users,nu]}));
    } else {
      const cambiaPass=form.password&&form.password.length>0;
      if(cambiaPass&&form.password!==form.password2){setFormErr("Las contraseñas no coinciden.");return;}
      setData(d=>({...d,users:d.users.map(u=>u.id!==form.id?u:{...u,nombre:form.nombre.trim(),usuario:form.usuario.trim(),email:form.email?.trim()||"",localId:parseInt(form.localId)||null,...(cambiaPass?{password:form.password}:{})})}));
    }
    setModal(null);
  };
  const toggle=id=>setData(d=>({...d,users:d.users.map(u=>u.id===id?{...u,activo:!u.activo}:u)}));
  return (
    <div>
        ...
        {manicuras.map(m=>{
          ...
          return (
            <Card key={m.id} style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              ...
              <Btn onClick={()=>toggle(m.id)} variant="ghost" size="sm" style={{color:m.activo?COLORS.danger:COLORS.success}}>{m.activo?"Desactivar":"Activar"}</Btn>
            </Card>
          );
        })}
      </div>
...
// ── ABM LOCALES ────────────────────────────────────────────────────
function ABMLocales({data,setData}){
  const [modal,setModal]=useState(null); const [form,setForm]=useState({});
  const openNew=()=>{setForm({nombre:"",direccion:""});setModal("new");};
  const openEdit=l=>{setForm({...l});setModal("edit");};
  const save=()=>{
    if(!form.nombre) return;
    if(modal==="new") setData(d=>({...d,locales:[...d.locales,{...form,id:Date.now()}]}));
    else setData(d=>({...d,locales:d.locales.map(l=>l.id===form.id?form:l)}));
    setModal(null);
  };
  const del=id=>{
    if(data.users.some(u=>u.localId===id)) return alert("Hay manicuras asignadas a este local.");
    setData(d=>({...d,locales:d.locales.filter(l=>l.id!==id)}));
  };
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
...
// ── PERFIL (admin y manicura) ──────────────────────────────────────
function MiPerfil({data,setData,user,setUser}){
  ...
  const save=()=>{
    ...
    const cambiaPass=form.password.length>0;
    if(cambiaPass&&form.password!==form.password2){setErr("Las contraseñas no coinciden.");return;}
    const updated={...user,nombre:form.nombre.trim(),email:form.email.trim(),...(cambiaPass?{password:form.password}:{})};
    setData(d=>({...d,users:d.users.map(u=>u.id===user.id?updated:u)}));
    setUser(updated); setOk(true);
  };
  return (
    <div>
      ...
      <Card style={{maxWidth:440}}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div>
            <label style={{fontSize:13,color:"var(--color-text-secondary)",display:"block",marginBottom:4}}>Nombre</label>
          ...
          <div>
            ...
            <Input type="email" value={form.email} onChange={v=>setForm(f=>({...f,email:v}))} placeholder="tu@mail.com"/>
          </div>
...
// ── CARGA HORARIOS ─────────────────────────────────────────────────
function CargaHorarios({data,setData,user}){
  const hoy=new Date();
  ...
  const setH=(dk,campo,val)=>{
    if(bloqueado) return;
    setData(d=>{
      const idx=d.horarios.findIndex(h=>h.userId===user.id&&h.fecha===dk);
      if(idx>=0){const u=[...d.horarios];u[idx]={...u[idx],[campo]:val};return {...d,horarios:u};}
      return {...d,horarios:[...d.horarios,{userId:user.id,fecha:dk,entrada:"",salida:"",trabaja:true,[campo]:val}]};
    });
  };
  ...
  return (
    <div>
      ...
      {bloqueado&&<div style={{background:COLORS.amberLight,color:COLORS.amber,padding:"10px 14px",borderRadius:8,marginBottom:16,fontSize:13}}>⚠️ Este período está bloqueado. No podés realizar modificaciones.</div>}
        ...
        {semanas.map((semana,si)=>{
          ...
          return (
            <Card key={si}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {semana.map(d=>{
                  const dk=dateKey(d); const h=getH(dk);
                  const dow=d.getDay(); const nomDia=DIAS_SEMANA[dow===0?6:dow-1];
                  return (
...
function GestionHorarios({data,setData}){
  const hoy=new Date();
  const [mes,setMes]=useState(hoy.getMonth()); const [anio,setAnio]=useState(hoy.getFullYear());
  const [manicuraId,setManicuraId]=useState(data.users.filter(u=>u.rol==="manicura")[0]?.id||null);
  const periodoKey=`${anio}-${String(mes+1).padStart(2,"0")}`;
  const bloqueado=data.periodosBloqueados.includes(periodoKey);
  const toggleBloqueo=()=>setData(d=>({...d,periodosBloqueados:bloqueado?d.periodosBloqueados.filter(p=>p!==periodoKey):[...d.periodosBloqueados,periodoKey]}));
  const dias=useMemo(()=>getDiasDelMes(anio,mes),[anio,mes]);
  const semanas=useMemo(()=>getSemanas(dias),[dias]);
  const getH=dk=>data.horarios.find(h=>h.userId===parseInt(manicuraId)&&h.fecha===dk)||{entrada:"",salida:"",trabaja:true};
  const setH=(dk,campo,val)=>setData(d=>{
    const uid=parseInt(manicuraId);
    const idx=d.horarios.findIndex(h=>h.userId===uid&&h.fecha===dk);
    if(idx>=0){const u=[...d.horarios];u[idx]={...u[idx],[campo]:val};return {...d,horarios:u};}
    return {...d,horarios:[...d.horarios,{userId:uid,fecha:dk,entrada:"",salida:"",trabaja:true,[campo]:val}]};
  });
                ...
                {semana.map(d=>{
                  ...
                  return (
                    <div key={dk} style={{display:"grid",gridTemplateColumns:"50px 1fr auto",gap:8,alignItems:"center"}}>
                      ...
                      {h.trabaja?(
                        <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                          <input type="time" value={h.entrada||""} onChange={e=>setH(dk,"entrada",e.target.value)} style={{border:"0.5px solid var(--color-border-secondary)",borderRadius:6,padding:"4px 8px",fontSize:13,background:"var(--color-background-primary)",color:"var(--color-text-primary)"}}/>
                          <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>a</span>
                          <input type="time" value={h.salida||""} onChange={e=>setH(dk,"salida",e.target.value)} style={{border:"0.5px solid var(--color-border-secondary)",borderRadius:6,padding:"4px 8px",fontSize:13,background:"var(--color-background-primary)",color:"var(--color-text-primary)"}}/>
                          {calcHoras(h.entrada,h.salida)>0&&<span style={{fontSize:12,color:COLORS.success,fontWeight:500}}>{calcHoras(h.entrada,h.salida).toFixed(1)}h</span>}
                        </div>
                      ):<span style={{fontSize:13,color:"var(--color-text-secondary)"}}>No trabaja</span>}
                      <button onClick={()=>setH(dk,"trabaja",!h.trabaja)} style={{background:h.trabaja?COLORS.pinkLight:COLORS.grayLight,color:h.trabaja?COLORS.pinkDark:"#666",border:"none",borderRadius:6,padding:"4px 10px",fontSize:12,cursor:"pointer",fontWeight:500}}>{h.trabaja?"Libre":"+ Agregar"}</button>
                    </div>
                  );
                })}
              </div>
...
// ── ASISTENCIA DIARIA ──────────────────────────────────────────────
function AsistenciaDiaria({data,setData}){
  const hoy=new Date();
  const [fecha,setFecha]=useState(dateKey(hoy));
  const [modal,setModal]=useState(null);
  const [formAus,setFormAus]=useState({}); const [formTarde,setFormTarde]=useState({});
  ...
  const setA=(uid,datos)=>setData(d=>{
    const ex=d.asistencias.findIndex(a=>a.userId===uid&&a.fecha===fecha);
    if(ex>=0){const u=[...d.asistencias];u[ex]={...u[ex],...datos};return {...d,asistencias:u};}
    return {...d,asistencias:[...d.asistencias,{userId:uid,fecha,...datos}]};
  });
    ...
    <div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                  ...
                  <div style={{flex:1,minWidth:100}}>
                    ...
                    <p style={{margin:0,fontSize:12,color:"var(--color-text-secondary)"}}>
                      ...
                      {a?.estado==="tarde"&&` | Real: ${a.entradaReal} – ${a.salidaReal}`}
                    </p>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    ...
                    {a&&<Btn onClick={()=>setData(d=>({...d,asistencias:d.asistencias.filter(a=>!(a.userId===m.id&&a.fecha===fecha))}))} variant="ghost" size="sm">Limpiar</Btn>}
                  </div>
                </div>
              </Card>
            );
      ...
      {modal==="tarde"&&(
        <Modal title="Registrar llegada tarde" onClose={()=>setModal(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <Btn onClick={()=>{setA(formTarde.uid,{estado:"tarde",entradaReal:formTarde.entrada,salidaReal:formTarde.salida});setModal(null);}} style={{flex:1,justifyContent:"center"}}>Guardar</Btn>
              <Btn onClick={()=>setModal(null)} variant="secondary" style={{flex:1,justifyContent:"center"}}>Cancelar</Btn>
            </div>
          </div>
        </Modal>
      ...
      {modal==="ausencia"&&(
        <Modal title="Registrar ausencia" onClose={()=>setModal(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <ModalSelect label="Motivo" value={formAus.motivo} onChange={v=>setFormAus(f=>({...f,motivo:v}))}>
              {MOTIVOS_AUSENCIA.map(m=><option key={m} value={m}>{m}</option>)}
            </ModalSelect>
            <label style={{display:"flex",alignItems:"center",gap:8,fontSize:14,cursor:"pointer"}}>
              <input type="checkbox" checked={formAus.certificado} onChange={e=>setFormAus(f=>({...f,certificado:e.target.checked}))}/>
            ...
            <div style={{display:"flex",gap:8}}>
              <Btn onClick={()=>{setA(formAus.uid,{estado:"ausente",motivo:formAus.motivo,certificado:formAus.certificado,tipoDoc:formAus.tipoDoc});setModal(null);}} style={{flex:1,justifyContent:"center"}}>Guardar</Btn>
              <Btn onClick={()=>setModal(null)} variant="secondary" style={{flex:1,justifyContent:"center"}}>Cancelar</Btn>
            </div>
          </div>
        </Modal>
...
// ── REPORTES (compartido admin y manicura) ─────────────────────────
function Reportes({data,user}){
  const hoy=new Date();
  const esAdmin=user.rol==="admin";
  const [tab,setTab]=useState("horas");
  const [filtroTipo,setFiltroTipo]=useState(esAdmin?"manicura":"manicura");
  ...
  const filtrarM=()=>{
    let base=esAdmin
      ?(filtroTipo==="manicura"?manicuras.filter(m=>m.id===parseInt(filtroId)):filtroTipo==="local"?manicuras.filter(m=>m.localId===parseInt(filtroId)):manicuras)
      :[data.users.find(u=>u.id===user.id)].filter(Boolean);
    if(filtroEstado!=="todos"){
      base=base.filter(m=>data.asistencias.some(a=>a.userId===m.id&&a.fecha>=fechaDesde&&a.fecha<=fechaHasta&&a.estado===filtroEstado));
    }
    return base;
  };
  const mF=filtrarM();
  ...
  const buildHorasReport=(m)=>{
    ...
    const semanasData=semanas.map((sem,si)=>{
      const diasData=sem.map(d=>{
        ...
        if(trabaja){
          if(!a) horasReal=0;
          else if(a.estado==="presente") horasReal=horasTeo;
  ...
  const TabBtn=({id,label})=>(
    <button onClick={()=>setTab(id)} style={{padding:"8px 16px",border:"none",borderRadius:8,cursor:"pointer",fontSize:14,fontWeight:500,background:tab===id?COLORS.pink:"transparent",color:tab===id?"#fff":"var(--color-text-secondary)"}}>{label}</button>
  );
  const estadoColor={presente:"success",tarde:"amber",ausente:"danger"};
  const estadoLabel={presente:"Presente",tarde:"Tarde",ausente:"Ausente"};
  ...
  return (
    <div>
      ...
      {/* Filtros — solo admin ve selector de manicura/local */}
      {esAdmin&&(
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        ...
        {(filtroSemana!=="todas"||filtroEstado!=="todos")&&(
          <button onClick={()=>{setFiltroSemana("todas");setFiltroEstado("todos");setExpandidos({});}} style={{background:COLORS.amberLight,color:COLORS.amber,border:"none",borderRadius:8,padding:"7px 12px",fontSize:13,fontWeight:500,cursor:"pointer"}}>✕ Limpiar filtros</button>
        )}
      </div>
      ...
      {tab==="horas"&&(
        <>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {mF.map(m=>{
              const r=buildHorasReport(m); const exp=expandidos[m.id];
              return (
      ...
      {tab==="asistencia"&&(
        <>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {mF.map(m=>{
              const r=buildAsistenciaReport(m); const exp=expandidos[m.id];
              return (
                <Card key={m.id} style={{padding:"0.875rem 1.25rem"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <Avatar nombre={r.nombre}/>
                      ...
                      {r.asist.length===0
                        ...
                        :<div style={{display:"flex",flexDirection:"column",gap:4}}>
                          <div style={{display:"grid",gridTemplateColumns:"80px 70px 1fr 1fr 80px",gap:8,padding:"4px 8px"}}>
                            {["Fecha","Estado","H. teórico","H. real","Detalle"].map(h=>(
                              <span key={h} style={{fontSize:11,fontWeight:500,color:"var(--color-text-secondary)",textTransform:"uppercase",letterSpacing:"0.04em"}}>{h}</span>
                            ))}
                          </div>
                          {r.asist.map(a=>{
...
export default function App(){
  ...
  useEffect(()=>{loadFromStorage().then(d=>{setDataRaw(d);setLoading(false);});},[]);

  const setData=useCallback((fn)=>{
    setDataRaw(prev=>{const next=typeof fn==="function"?fn(prev):fn;saveToStorage(next);return next;});
  },[]);

  if(loading) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}><p style={{color:"var(--color-text-secondary)",fontSize:14}}>Cargando...</p></div>;

  if(!user) return <Login onLogin={u=>{setUser(u);setSeccion(u.rol==="admin"?"asistencia":"horarios");}} data={data} setData={setData}/>;
  ...
  const renderSeccion=()=>{
    if(seccion==="asistencia") return <AsistenciaDiaria data={data} setData={setData}/>;
    if(seccion==="horarios_admin") return <GestionHorarios data={data} setData={setData}/>;
    if(seccion==="reportes") return <Reportes data={data} user={user}/>;
    if(seccion==="manicuras") return <ABMManicuras data={data} setData={setData}/>;
    if(seccion==="locales") return <ABMLocales data={data} setData={setData}/>;
    if(seccion==="horarios") return <CargaHorarios data={data} setData={setData} user={user}/>;
    if(seccion==="perfil") return <MiPerfil data={data} setData={setData} user={user} setUser={setUser}/>;
    return null;
  };
