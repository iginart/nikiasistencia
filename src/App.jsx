...
const patchOrPost = async (table, q, data) => {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${q}`, { method: "PATCH", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify(data) });
  const t = await r.text(); const res = t ? JSON.parse(t) : [];
  if (!res || res.length === 0) return sb(table, { method: "POST", body: JSON.stringify(data) });
  return res;
};
const api = {
  getUsers: () => sb("users?select=*&order=id"),
  ...
  deleteAsistencia: (userId, fecha) => sb(`asistencias?user_id=eq.${userId}&fecha=eq.${fecha}`, { method: "DELETE", prefer: "" }),
  getPeriodos: () => sb("periodos_bloqueados?select=*"),
  createPeriodo: (p) => sb("periodos_bloqueados", { method: "POST", body: JSON.stringify({ periodo: p }) }),
  deletePeriodo: (p) => sb(`periodos_bloqueados?periodo=eq.${encodeURIComponent(p)}`, { method: "DELETE", prefer: "" }),
  getTokens: () => sb("reset_tokens?select=*"),
  createToken: (d) => sb("reset_tokens", { method: "POST", body: JSON.stringify(d) }),
  deleteToken: (t) => sb(`reset_tokens?token=eq.${encodeURIComponent(t)}`, { method: "DELETE", prefer: "" }),
  deleteTokenByUser: (id) => sb(`reset_tokens?user_id=eq.${id}`, { method: "DELETE", prefer: "" }),
  getFeriados: () => sb("feriados?select=*"),
  createFeriado: (d) => sb("feriados", { method: "POST", body: JSON.stringify(d) }),
  deleteFeriado: (fecha) => sb(`feriados?fecha=eq.${fecha}`, { method: "DELETE", prefer: "" }),
};
...
const COLORS = { pink: "#d4537e", pinkLight: "#fbeaf0", pinkDark: "#72243e", gray: "#888780", grayLight: "#f1efe8", success: "#639922", successLight: "#eaf3de", danger: "#e24b4a", dangerLight: "#fcebeb", amber: "#ba7517", amberLight: "#faeeda", info: "#185fa5", infoLight: "#e6f1fb" };
const DIAS_SEMANA = ["Lun","Mar","Mié","Jue","Vie","Sáb"];
const MESES = ["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];
const MOTIVOS_AUSENCIA = ["Enfermedad","Personal","Trámite","Licencia","Otro"];
  ...
  const renderMensual = () => {
    ...
    return <div style={{ flex:1,overflow:"auto" }}>
      ...
      {sems.map((semana,si) => {
        ...
        return <div key={si} style={{ display:"grid",gridTemplateColumns:"repeat(6,1fr) 80px",borderBottom:"1px solid var(--color-border-secondary)",minHeight:90 }}>
      ...
      <div style={{ display:"flex",height:560,border:"0.5px solid var(--color-border-tertiary)",borderRadius:12,overflow:"hidden",background:"var(--color-background-primary)" }}>
        ...
        <div style={{ flex:1,display:"flex",flexDirection:"column",overflow:"hidden" }}>
          <div style={{ padding:"6px 10px",borderBottom:"0.5px solid var(--color-border-tertiary)",display:"flex",alignItems:"center",gap:8 }}>
            <button onClick={()=>setNavVisible(v=>!v)} title={navVisible?"Ocultar panel":"Mostrar panel"} style={{ background:"none",border:"0.5px solid var(--color-border-secondary)",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:12,color:"var(--color-text-secondary)",flexShrink:0,whiteSpace:"nowrap" }}>
              {navVisible?"‹ Ocultar":"Panel ›"}
            </button>
            <div style={{ width:1,height:18,background:"var(--color-border-tertiary)",flexShrink:0,margin:"0 2px" }}/>
            ...
            {vista==="semana"&&!bloqueado&&isMobile&&<span style={{ marginLeft:"auto",fontSize:11,color:"var(--color-text-secondary)",opacity:0.7 }}>Tocá un día para editar</span>}
          </div>
...
export default function App() {
  ...
  const renderSeccion = () => {
    ...
    if (seccion === "horarios") return < data={data} reloadData={reloadData} user={user} />;
