  ...
  style.textContent = `
    ...
    * {
    ...
    @media (max-width: 640px) {
      ...
      .niki-history-row > input[placeholder="Opcional"],
      .niki-history-row > input[placeholder="Observación"] {
        grid-column: 1 / -1;
      }
      .niki-history-row > button {
        width: 100% !important;
...
function mergeWorkPeriods(periods) {
  const normalized = (periods || []).map(p => {
    const start = parseDateLocal(p.fechaInicio || p.fecha_inicio);
    const end = parseDateLocal(p.fechaFin || p.fecha_fin) || new Date(2099,11,31,12,0,0,0);
    return start && end >= start ? { start, end } : null;
  }).filter(Boolean).sort((a,b)=>a.start-b.start);
  const merged = [];
  normalized.forEach(p => {
    const last = merged[merged.length-1];
    if (!last || p.start > addDaysLocal(dateKey(last.end), 1)) merged.push({ ...p });
    else if (p.end > last.end) last.end = p.end;
  });
  return merged;
}
function getMinimumGuaranteeStatus(histories, userId, localId, weekStartKey, weekEndKey) {
  if (!userId || !localId || !weekStartKey || !weekEndKey) return { eligible:false, endDateKey:"", activeInWeek:false };
  const periods = mergeWorkPeriods((histories || []).filter(h => parseInt(h.userId)===parseInt(userId) && parseInt(h.localId)===parseInt(localId)));
  if (!periods.length) return { eligible:false, endDateKey:"", activeInWeek:false };
  const firstStart = periods[0].start;
  const requiredDays = diffCalendarDays(firstStart, addCalendarMonthsClamped(firstStart, 2));
  let remaining = requiredDays;
  let endDate = null;
  for (const p of periods) {
    const available = diffCalendarDays(p.start, addDaysLocal(dateKey(p.end), 1));
    if (remaining <= available) { endDate = addDaysLocal(dateKey(p.start), remaining); break; }
    remaining -= available;
  }
  if (!endDate) {
    const last = periods[periods.length-1];
    endDate = addDaysLocal(dateKey(last.end), remaining);
  }
  const weekStart = parseDateLocal(weekStartKey);
  const weekEnd = parseDateLocal(weekEndKey);
  const activeInWeek = periods.some(p => p.start <= weekEnd && p.end >= weekStart);
  const started = firstStart <= weekEnd;
...
const HELP_TOPICS = [
  {
    ...
    summary: "Cómo aceptar la invitación, registrar y validar tu email, crear una contraseña segura e ingresar con usuario o correo electrónico.",
    sections: [
      {
        heading: "1. Recibí la invitación",
        text: "Cuando administración crea tu usuario, recibís una invitación en el correo informado. Abrí ese mensaje y seguí el enlace para comenzar. La invitación es personal: no la compartas con otra persona.",
        bullets: [
          "Revisá que el mensaje corresponda a NIKI OS.",
          "Abrí el enlace desde el mismo correo que fue informado para tu usuario.",
          "Si no encontrás el mensaje, revisá spam, promociones o correo no deseado.",
        ],
        note: "Si la invitación venció o el correo informado es incorrecto, pedile a tu encargada o a administración que la reenvíen.",
      },
      {
        heading: "2. Completá tus datos de seguridad",
        text: "En el primer ingreso, NIKI OS puede pedirte que cargues un correo electrónico y reemplaces la contraseña temporal. El correo es obligatorio porque se utiliza para validar tu identidad y recuperar el acceso si olvidás la contraseña.",
        bullets: [
          "Ingresá un correo real al que tengas acceso.",
          "Creá una contraseña de al menos 8 caracteres.",
          "No podés usar la contraseña temporal niki123 como contraseña definitiva.",
          "Escribí nuevamente la contraseña para confirmar que no tenga errores.",
          "Presioná Enviar código y continuar.",
        ],
        image: "/ayuda/primer-ingreso-seguridad.png",
        imageAlt: "Pantalla de NIKI OS para completar el email y cambiar la contraseña temporal",
        imageCaption: "Primer ingreso: email obligatorio y cambio de contraseña temporal.",
      },
      {
        heading: "3. Validá tu correo electrónico",
        text: "Después de enviar tus datos, vas a recibir un código de validación en tu casilla. Ingresalo en NIKI OS para confirmar que el correo existe y que tenés acceso. Cada correo puede estar asociado a una sola persona.",
        bullets: [
          "Esperá unos minutos y revisá la bandeja de entrada.",
          "Revisá también spam o correo no deseado.",
          "Confirmá que la dirección ingresada esté escrita correctamente.",
          "No compartas el código con otras personas.",
        ],
        note: "Si el sistema indica que el correo ya está en uso, administración debe revisar los usuarios existentes antes de continuar.",
      },
      {
        heading: "4. Ingresá a NIKI OS",
        text: "Una vez completado el primer ingreso, podés entrar usando tu nombre de usuario o tu correo electrónico, junto con tu contraseña. El usuario y el email no distinguen mayúsculas y minúsculas; la contraseña sí.",
        bullets: [
          "Escribí tu usuario o email en el primer campo.",
          "Ingresá la contraseña exactamente como la creaste.",
          "Podés tocar el ícono del ojo para verla y comprobar que esté bien escrita.",
          "Presioná Ingresar.",
        ],
        image: "/ayuda/login.png",
        imageAlt: "Pantalla de ingreso a NIKI OS con usuario o email y contraseña",
        imageCaption: "Ingreso habitual: podés usar tu usuario o tu email.",
      },
      {
        heading: "5. Si olvidaste tu contraseña",
        text: "Desde la pantalla de ingreso, tocá ¿Olvidaste tu contraseña? Escribí tu usuario o email y presioná Enviar instrucciones. Vas a recibir un enlace para crear una contraseña nueva.",
        bullets: [
          "Podés identificarte con tu usuario o con el correo registrado.",
          "Usá solamente el último enlace recibido.",
          "Si no llega el mensaje, revisá spam y verificá que el email sea el correcto.",
        ],
        image: "/ayuda/recuperar-password.png",
        imageAlt: "Pantalla de NIKI OS para solicitar instrucciones de recuperación de contraseña",
        imageCaption: "Recuperación de contraseña mediante usuario o email.",
      },
      {
        heading: "6. Si no podés ingresar",
        bullets: [
          "Revisá que no haya espacios antes o después del usuario o email.",
          "Comprobá mayúsculas y minúsculas de la contraseña.",
          "Usá la opción ¿Olvidaste tu contraseña? si no la recordás.",
  ...
  },
  {
    ...
    icon: "🗓️",
    profiles: ["casa_matriz", "encargada", "manicura"],
    summary: "Cómo cargar, repetir, revisar y ajustar horarios en las vistas semanal y mensual.",
    sections: [
      {
        heading: "Cuándo se cargan los horarios",
        text: "Los horarios se cargan con un mes de anticipación, durante la primera semana del mes anterior. Por ejemplo, durante la primera semana de julio deben quedar cargados los horarios de agosto.",
        note: "Después del período de carga, el mes se bloquea. Solo se vuelve a habilitar de manera excepcional cuando existe una corrección justificada.",
      },
      {
        heading: "Vista semanal",
        text: "La vista Semana permite trabajar con cada día en una grilla horaria. Cada bloque muestra la hora de entrada, la hora de salida y la cantidad de horas del día.",
        bullets: [
          "Usá las flechas del período para pasar a la semana anterior o siguiente.",
          "El total de la derecha muestra las horas cargadas para toda la semana.",
          "El resumen lateral indica cuántos días están cargados y el total de horas visibles.",
          "Para agregar un horario, hacé clic en una celda vacía y luego confirmá entrada y salida.",
        ],
        image: "/ayuda/horarios-vista-semanal.png",
        imageAlt: "Vista semanal de la pantalla de horarios",
        imageCaption: "Vista semanal con bloques diarios y total de horas de la semana.",
      },
      {
        heading: "Ajustar un horario arrastrando el bloque",
        bullets: [
          "Arrastrá el bloque completo hacia arriba o hacia abajo para mover el horario manteniendo su duración.",
          "Arrastrá el borde superior para cambiar la hora de entrada.",
          "Arrastrá el borde inferior para cambiar la hora de salida.",
          "Al pasar el puntero sobre un bloque se muestra el detalle con manicura, día, horario y estado de asistencia.",
        ],
        image: "/ayuda/horarios-arrastrar-tooltip.png",
        imageAlt: "Detalle emergente de un bloque de horario",
        imageCaption: "El detalle emergente ayuda a verificar el horario antes de modificarlo.",
      },
      {
        heading: "Editar o eliminar un horario ya cargado",
        text: "Podés abrir un horario haciendo clic sobre el bloque o sobre el día en la vista mensual. El sistema muestra la hora de entrada y salida para modificarla.",
        bullets: [
          "Presioná Guardar para confirmar el nuevo horario.",
          "Presioná Eliminar solamente cuando el día no deba tener horario.",
          "Presioná Cancelar para cerrar sin guardar cambios.",
          "Si el horario ya estaba guardado, el sistema puede pedir una confirmación antes de modificarlo o eliminarlo.",
        ],
        image: "/ayuda/horarios-editar-modal.png",
        imageAlt: "Ventana para editar un horario",
        imageCaption: "Ventana de edición con entrada, salida, guardar, eliminar y cancelar.",
      },
      {
        heading: "Vista mensual",
        text: "La vista Mes permite controlar el mes completo de una sola vez. Cada fila representa una semana y la columna Sem. muestra el total de horas de esa semana.",
        bullets: [
          "Los días con horario aparecen resaltados y muestran entrada, salida y horas.",
          "Los días vacíos muestran un signo + para agregar un horario.",
          "El resumen lateral muestra los días cargados y las horas totales del mes.",
          "Esta vista es útil para detectar rápidamente días sin cargar o semanas con una cantidad de horas incorrecta.",
        ],
        image: "/ayuda/horarios-vista-mensual.png",
        imageAlt: "Vista mensual de la pantalla de horarios",
        imageCaption: "Vista mensual con los totales semanales y el resumen general del mes.",
      },
      {
        heading: "Repetir semana anterior",
        text: "Esta función copia los horarios de la semana anterior y evita volver a cargar día por día cuando se repite el mismo esquema.",
        bullets: [
          "En la vista Semana, el botón Repetir semana anterior copia los horarios de los seis días visibles.",
          "En la vista Mes, el botón con la flecha ubicado en la columna Sem. copia la semana anterior solamente sobre esa fila.",
          "Antes de copiar, el sistema informa cuántos horarios se cargarán, cuántos se reemplazarán y qué días se omitirán.",
          "No se copian días bloqueados, sin permiso de edición o con asistencia ya registrada.",
          "Después de copiar, revisá el total semanal y ajustá los días que sean diferentes.",
        ],
      },
      {
        heading: "Feriados",
        text: "Los feriados se visualizan con un fondo y una identificación diferente dentro del calendario. El horario puede seguir apareciendo si fue cargado, pero el día queda claramente marcado para facilitar el control.",
        bullets: [
          "En la vista semanal, el encabezado del día muestra la indicación de feriado.",
          "En la vista mensual, la celda del día aparece destacada.",
          "La marcación de feriados es administrada por los perfiles habilitados.",
        ],
      },
      {
        heading: "Períodos bloqueados",
        text: "Cuando el período está bloqueado no se pueden guardar, mover ni eliminar horarios. Al abrir un día bloqueado, el sistema muestra el aviso correspondiente.",
        bullets: [
          "Si detectás un error después del bloqueo, avisá a tu encargada o a administración.",
          "La reapertura debe ser excepcional y solo por el tiempo necesario para corregir.",
          "Una vez realizado el ajuste, el período debe volver a bloquearse.",
        ],
        image: "/ayuda/horarios-periodo-bloqueado.png",
        imageAlt: "Aviso de período de horarios bloqueado",
        imageCaption: "Un período bloqueado no permite realizar modificaciones.",
      },
      {
        heading: "Control final antes de terminar",
        bullets: [
          "Confirmá que estás trabajando en el mes correcto.",
          "Revisá que todos los días de trabajo tengan horario.",
          "Controlá los totales semanales y el total del mes.",
          "Verificá feriados, francos y cambios excepcionales.",
          "No esperes al bloqueo para informar una corrección.",
        ],
      },
    ],
  },
  {
    ...
    summary: "Cómo controlar la apertura y el cierre de la carga de horarios por local, mes y manicura.",
    sections: [
      {
        heading: "Para qué sirve el bloqueo",
        text: "La pantalla Bloqueo de horarios controla si cada manicura puede cargar o modificar sus horarios para un local y un mes determinados. Se utiliza para cerrar la planificación una vez finalizado el período de carga y evitar cambios posteriores sin control.",
        note: "Este bloqueo solo afecta la carga y modificación de horarios de trabajo. No bloquea turnos, disponibilidad para clientes ni otras funciones de la agenda.",
      },
      {
        heading: "Cuándo debe bloquearse un mes",
        text: "Los horarios se cargan con un mes de anticipación durante la primera semana del mes anterior. Por ejemplo, durante la primera semana de julio se cargan los horarios de agosto. Una vez revisada la carga, el mes debe bloquearse.",
        bullets: [
          "Antes de bloquear, verificá que todas las manicuras hayan completado el mes.",
          "Revisá que no queden días laborales sin horario.",
          "Controlá los totales semanales y el total mensual.",
          "Confirmá que feriados, francos y cambios excepcionales estén correctamente reflejados.",
        ],
      },
      {
        heading: "Cómo leer la pantalla",
        text: "La parte superior permite elegir el local, el mes, el estado y una manicura específica. Debajo se muestra un resumen y el listado completo de personas visibles.",
        bullets: [
          "Local: define sobre qué sucursal estás trabajando.",
          "Mes: determina el período que se va a habilitar o bloquear.",
          "Estado: permite mostrar todas, solo las habilitadas o solo las bloqueadas.",
          "Buscar manicura: filtra por nombre o usuario.",
          "Bloqueadas: muestra cuántas manicuras están bloqueadas sobre el total visible.",
        ],
        image: "/ayuda/bloqueo-horarios.png",
        imageAlt: "Pantalla de bloqueo de horarios por local y mes",
        imageCaption: "Pantalla de control con filtros, resumen y acciones individuales o masivas.",
      },
      {
        heading: "Estados posibles",
        bullets: [
          "Habilitado: la manicura puede cargar, mover, editar o eliminar horarios del mes seleccionado, siempre que no exista otra restricción.",
          "Bloqueado: la manicura ya no puede modificar sus horarios de ese mes.",
          "El estado se controla por manicura, local y mes. Una misma persona puede estar bloqueada en un período y habilitada en otro.",
        ],
      },
      {
        heading: "Bloquear una manicura",
        text: "Usá el botón Bloquear ubicado al final de la fila cuando una manicura ya terminó de cargar sus horarios y la información fue revisada.",
        bullets: [
          "Verificá primero el local y el mes seleccionados.",
          "Confirmá que la fila corresponda a la manicura correcta.",
          "Presioná Bloquear.",
          "Revisá que el estado cambie de Habilitado a Bloqueado.",
          "La cantidad del resumen Bloqueadas debe actualizarse.",
        ],
      },
      {
        heading: "Habilitar una manicura de manera excepcional",
        text: "Si después del bloqueo aparece una corrección justificada, se puede volver a habilitar solamente a la manicura afectada.",
        bullets: [
          "Validá el motivo de la corrección antes de habilitar.",
          "Seleccioná el local y el mes correctos.",
          "Buscá a la manicura por nombre o usuario.",
          "Presioná Habilitar en su fila.",
          "Pedile que realice únicamente el ajuste necesario.",
          "Una vez verificado el cambio, volvé a bloquear el período.",
        ],
        note: "La habilitación posterior al cierre debe ser excepcional y durar solo el tiempo necesario para corregir.",
      },
      {
        heading: "Bloquear o habilitar varias manicuras",
        text: "Los botones de la parte superior aplican la acción sobre las manicuras que están visibles según los filtros utilizados.",
        bullets: [
          "Bloquear visibles: bloquea todas las manicuras mostradas en el listado.",
          "Habilitar visibles: habilita todas las manicuras mostradas en el listado.",
          "Antes de usar una acción masiva, revisá especialmente los filtros Local, Mes y Estado.",
          "Podés combinar el filtro de estado con la búsqueda para trabajar solo con un grupo específico.",
          "Después de ejecutar la acción, verificá el contador de bloqueadas y algunas filas del listado.",
        ],
        note: "La palabra visibles es importante: la acción se aplica al conjunto que quedó en pantalla luego de usar los filtros.",
      },
      {
        heading: "Flujo recomendado de cierre",
        bullets: [
          "Durante la primera semana del mes, las manicuras cargan el mes siguiente.",
          "La encargada revisa días, horarios y totales.",
          "Se solicitan las correcciones necesarias mientras el período sigue habilitado.",
          "Cuando la información está completa, se bloquean las manicuras del local.",
          "Se controla que el resumen indique el total esperado de bloqueadas.",
          "Cualquier reapertura posterior se realiza de forma individual y excepcional.",
        ],
      },
      {
        heading: "Errores que conviene evitar",
        bullets: [
          "Bloquear sin revisar el mes seleccionado.",
          "Usar Bloquear visibles con filtros incorrectos.",
          "Habilitar a todas cuando solo una manicura necesita corregir.",
          "Dejar un período abierto después de completar una corrección.",
          "Confundir el bloqueo de horarios con el bloqueo de turnos o disponibilidad.",
        ],
      },
    ],
  },
  {
    ...
    icon: "📋",
    profiles: ["casa_matriz", "encargada"],
    summary: "Cómo registrar presentes, llegadas tarde y ausencias, y cómo controlar horas teóricas y reales.",
    sections: [
      {
        heading: "Por qué es importante registrar la asistencia",
        text: "La asistencia diaria deja constancia de lo que realmente ocurrió en cada jornada. Permite comparar el horario planificado con la asistencia real, detectar llegadas tarde o ausencias y mantener información confiable para los reportes.",
        bullets: [
          "Ayuda a controlar el cumplimiento de los horarios cargados.",
          "Permite distinguir horas planificadas de horas realmente trabajadas.",
          "Evita que un horario ya utilizado para asistencia se modifique sin control.",
          "Aporta información para aplicar correctamente los criterios semanales de comisiones.",
        ],
        note: "La asistencia debe registrarse con precisión. Un estado incorrecto puede afectar los controles de horas y, según la configuración vigente, el porcentaje de comisión de la semana.",
      },
      {
        heading: "De dónde salen los datos teóricos",
        text: "Las horas teóricas se calculan a partir de los horarios cargados previamente en la pantalla Horarios. Por eso es importante que el calendario esté completo y revisado antes de registrar asistencia.",
        bullets: [
          "La hora de entrada teórica surge del inicio del bloque cargado para ese día.",
          "La hora de salida teórica surge del final del bloque.",
          "Las horas teóricas del día son la diferencia entre entrada y salida.",
          "Los totales semanales suman los horarios planificados de todos los días visibles.",
          "Si un día no tiene horario cargado, no puede compararse correctamente con la asistencia real.",
        ],
      },
      {
        heading: "Pantalla de asistencia diaria",
        text: "La pantalla muestra las manicuras agrupadas por local para la fecha seleccionada. En cada fila se ve el horario previsto y las acciones disponibles para registrar el resultado del día.",
        bullets: [
          "Usá el selector de local para revisar una sucursal específica o todos los locales habilitados.",
          "Usá la fecha para consultar o registrar otro día.",
          "Los indicadores superiores resumen cuántas personas tienen horario, están presentes, llegaron tarde, están ausentes o siguen pendientes.",
          "Pendiente significa que todavía no se registró ningún estado para esa persona.",
          "El botón Limpiar elimina el registro de asistencia del día y vuelve a dejarlo pendiente.",
        ],
        image: "/ayuda/asistencia-diaria.png",
        imageAlt: "Pantalla de asistencia diaria agrupada por local",
        imageCaption: "Cada fila muestra la manicura, su horario planificado y las opciones de registro.",
      },
      {
        heading: "Registrar una presencia",
        text: "Usá la opción Presente cuando la manicura cumplió su jornada sin una llegada tarde que deba quedar registrada.",
        bullets: [
          "El sistema toma como referencia el horario teórico cargado.",
          "El estado queda identificado en verde.",
          "Una vez registrada, la fila deja de aparecer como pendiente.",
          "Si se marcó por error, usá Limpiar y registrá nuevamente el estado correcto.",
        ],
      },
      {
        heading: "Registrar una llegada tarde",
        text: "Usá la opción Tarde cuando la hora real de entrada sea posterior a la hora planificada. La ventana permite informar entrada y salida reales.",
        bullets: [
          "Horario real de entrada: indicá la hora efectiva de llegada.",
          "Horario real de salida: indicá la hora efectiva de finalización.",
          "Revisá ambos valores antes de guardar.",
          "Las horas reales se calculan con estos datos y luego se comparan con las teóricas.",
          "La cantidad de llegadas tarde puede intervenir en el criterio semanal de comisión, según la configuración general o individual.",
        ],
        image: "/ayuda/asistencia-llegada-tarde.png",
        imageAlt: "Ventana para registrar una llegada tarde",
        imageCaption: "La llegada tarde requiere informar los horarios reales de entrada y salida.",
      },
      {
        heading: "Registrar una ausencia",
        text: "Usá la opción Ausente cuando la manicura no asistió. El sistema solicita un motivo y permite indicar si presenta documentación.",
        bullets: [
          "Elegí el motivo que mejor represente el caso.",
          "Marcá Presenta documentación cuando exista certificado u otro comprobante.",
          "Si presenta documentación, seleccioná el tipo correspondiente.",
          "Guardá solamente cuando la información esté verificada.",
          "Las faltas justificadas y no justificadas pueden tratarse de manera diferente en los criterios de comisión.",
        ],
        image: "/ayuda/asistencia-ausencia.png",
        imageAlt: "Ventana para registrar una ausencia",
        imageCaption: "La ausencia incluye motivo, documentación y tipo de comprobante.",
      },
      {
        heading: "Relación entre asistencia y comisiones",
        text: "El reporte de comisiones puede evaluar cada semana usando las horas objetivo, las horas trabajadas, las llegadas tarde y las ausencias. El resultado depende de la configuración general y, cuando existe, de la configuración particular de cada manicura.",
        bullets: [
          "Las horas reales ayudan a verificar si se alcanzó el objetivo semanal.",
          "Las llegadas tarde se comparan con el máximo permitido.",
          "Las faltas no justificadas se comparan con el límite configurado.",
          "Las faltas justificadas pueden contarse o no, según la regla vigente.",
          "El sistema puede aplicar el porcentaje base o el porcentaje reducido.",
          "Una selección manual o una comisión definitiva puede reemplazar el criterio automático cuando corresponda.",
        ],
        note: "Registrar asistencia no calcula por sí solo toda la comisión. Es una de las fuentes utilizadas junto con horarios, configuración, criterios semanales y otros ajustes.",
      },
      {
        heading: "Reporte Horas y asistencia",
        text: "Este reporte permite comparar por semana las horas teóricas y las horas reales de cada manicura. También permite revisar el detalle día por día.",
        bullets: [
          "Elegí el local, la semana, el estado, el mes y el año.",
          "Horas teóricas: provienen del calendario de horarios.",
          "Horas reales: provienen de los registros de asistencia.",
          "La cantidad de días indica cuántas jornadas forman parte del período mostrado.",
          "Ver detalle despliega la información diaria de la manicura.",
        ],
        image: "/ayuda/horas-asistencia-resumen.png",
        imageAlt: "Resumen semanal de horas teóricas y reales",
        imageCaption: "El resumen permite detectar rápidamente diferencias entre lo planificado y lo realmente trabajado.",
      },
      {
        heading: "Cómo leer el detalle semanal",
        text: "Al desplegar una manicura, el reporte muestra cada día de la semana, el horario teórico, las horas teóricas, las horas reales y el estado registrado.",
        bullets: [
          "Un tilde verde indica una jornada registrada como presente.",
          "Libre identifica un día sin horario laboral.",
          "Las llegadas tarde y ausencias se muestran con su estado correspondiente.",
          "El total superior permite comparar rápidamente horas teóricas y reales.",
          "Si los valores no coinciden, revisá el detalle para encontrar el día que genera la diferencia.",
        ],
        image: "/ayuda/horas-asistencia-detalle.png",
        imageAlt: "Detalle semanal de horas y asistencia",
        imageCaption: "El detalle muestra la composición diaria de los totales teóricos y reales.",
      },
      {
        heading: "Correcciones y cuidados",
        bullets: [
          "Confirmá siempre el local y la fecha antes de registrar.",
          "No marques presente si todavía no verificaste la asistencia.",
          "Usá Tarde cuando corresponda, en lugar de corregir el horario teórico.",
          "No modifiques el horario planificado para hacer coincidir una asistencia real.",
          "Si el registro fue incorrecto, usá Limpiar y volvé a cargarlo.",
          "Si el día tiene asistencia registrada, el horario puede quedar protegido para evitar inconsistencias.",
          "Ante una corrección de un período cerrado, consultá con administración.",
        ],
      },
    ],
  },
        ...
        <Card>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
            {selected.sections.map((section, idx) => <section key={idx} style={{ borderTop:idx===0?"none":"1px solid rgba(120,120,120,0.14)",paddingTop:idx===0?0:18 }}>
              ...
              {section.image && <figure style={{ margin:"16px 0 0",display:"flex",flexDirection:"column",alignItems:"center",gap:8 }}>
                <img
                  src={section.image}
                  alt={section.imageAlt || section.heading}
                  loading="lazy"
                  style={{ display:"block",width:"100%",maxWidth:430,height:"auto",objectFit:"contain",borderRadius:16,border:"1px solid rgba(114,36,62,0.14)",background:"#fff",boxShadow:"0 10px 28px rgba(0,0,0,0.08)" }}
                />
                {section.imageCaption && <figcaption style={{ maxWidth:430,textAlign:"center",fontSize:12,lineHeight:1.45,color:"var(--color-text-secondary)" }}>{section.imageCaption}</figcaption>}
              </figure>}
            </section>)}
          </div>
        </Card>
  ...
  const renderComisiones = () => {
    ...
    const abrirConfigComisiones = () => {
      setConfigComisionesDraft({
        ...
        manicuras: data.users.filter(m=>m.rol==="manicura").flatMap(m => {
          const localIds = Array.from(new Set([
            ...(m.localId ? [m.localId] : []),
            ...(data.manicuraHistorialLocales || []).filter(h=>parseInt(h.userId)===parseInt(m.id)).map(h=>h.localId),
            ...(data.comisionesManicuraConfig || []).filter(c=>parseInt(c.userId)===parseInt(m.id) && c.localId).map(c=>c.localId),
          ].filter(Boolean)));
          return (localIds.length ? localIds : [0]).map(localIdValue => {
            const cfg = getConfigManicura(m.id, localIdValue) || {};
            const localObj = data.locales.find(l=>parseInt(l.id)===parseInt(localIdValue));
            return {
              userId:m.id,
              localId:localIdValue || 0,
              nombre:m.nombre,
              local:localObj?.nombre || "Sin local",
    ...
    adelantos.forEach(a => {
      ...
      if (!manicurasPagoMap.has(key)) manicurasPagoMap.set(key, { userId:a.userId || null, localId:a.localId || m?.localId || null, nombre:m?.nombre || "Sin manicura", local:l?.nombre || "", comision:0, adelantos:0 });
    ...
    const resumenMapComisionesSeed = new Map();
    if (!sinSemanaComisiones && localComisionesSeleccionado) {
      const candidateIds = new Set((data.manicuraHistorialLocales || [])
        .filter(h=>parseInt(h.localId)===parseInt(localComisionesSeleccionado))
        .map(h=>parseInt(h.userId)));
      data.users.filter(m=>m.rol==="manicura" && m.activo!==false).forEach(m=>{
        if (parseInt(m.localId)===parseInt(localComisionesSeleccionado) || candidateIds.has(parseInt(m.id))) {
          const g = garantiaMinimaInfo(m.id, localComisionesSeleccionado);
          if (g.eligible && (manicuraComisiones === "todas" || parseInt(manicuraComisiones)===parseInt(m.id))) {
            resumenMapComisionesSeed.set(m.id, { userId:m.id, localId:localComisionesSeleccionado, nombre:m.codigoExterno||m.nombre||"Sin manicura", local:localNameById.get(localComisionesSeleccionado)||"", precio:0, comisionBase:0, comision35:0, comisionReal:0, comisionDefinitiva:0, porcentajeAplicado:40, porcentajeAutomatico:40, criterioGuardado:false, horasTeoricas:0, faltas:0, garantias:0, adelantos:0, neto:0, servicios:0, garantiasQty:0, minimoGarantizado:g.minimum, minimoFin:g.endDateKey, minimoElegible:g.eligible, minimoAplica:false });
          }
        }
      });
    }
      ...
      {configComisionesDraft&&<Modal title="Configuración de comisiones" onClose={()=>setConfigComisionesDraft(null)} width={980}>
        <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:8,background:"var(--color-background-secondary)",borderRadius:10,padding:10 }}>
            <Select value={configComisionesFiltros.local} onChange={v=>setConfigComisionesFiltros(f=>({...f,local:v}))}><option value="">Todos los locales</option>{data.locales.map(l=><option key={l.id} value={l.id}>{l.nombre}</option>)}</Select>
            <Select value={configComisionesFiltros.tipoLocal} onChange={v=>setConfigComisionesFiltros(f=>({...f,tipoLocal:v}))}><option value="">Todos los tipos</option><option value="propio">Propio</option><option value="franquicia">Franquicia</option></Select>
            <Select value={configComisionesFiltros.zona} onChange={v=>setConfigComisionesFiltros(f=>({...f,zona:v}))}><option value="">Todas las zonas</option><option value="estandar">Estándar</option><option value="premium">Premium</option><option value="exclusiva">Exclusiva</option></Select>
            <Select value={configComisionesFiltros.manicura} onChange={v=>setConfigComisionesFiltros(f=>({...f,manicura:v}))}><option value="">Todas las manicuras</option>{manicuras.map(m=><option key={m.id} value={m.id}>{m.nombre}</option>)}</Select>
            <Select value={configComisionesFiltros.estado} onChange={v=>setConfigComisionesFiltros(f=>({...f,estado:v}))}><option value="">Activas e inactivas</option><option value="activas">Activas</option><option value="inactivas">Inactivas</option></Select>
